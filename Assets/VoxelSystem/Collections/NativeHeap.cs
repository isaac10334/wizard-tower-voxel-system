...
namespace Unity.Collections
{
        ...
        where U : unmanaged, IComparer<T>
    {
                ...
                unsafe
                {
                    ...
                    if (value < Data->Count)
                    {
                        throw new ArgumentException(
                            $"Capacity of {value} cannot be smaller than count of {Data->Count}.");
                    }
#endif
                    TableValue* newTable =
                        (TableValue*)Malloc(SizeOf<TableValue>() * value, AlignOf<TableValue>(), Allocator);
            ...
            int disposeSentinelStackDepth)
        {
            ...
            Data->Heap = (HeapNode<T>*)Malloc(SizeOf<HeapNode<T>>() * initialCapacity, AlignOf<HeapNode<T>>(),
                allocator);
